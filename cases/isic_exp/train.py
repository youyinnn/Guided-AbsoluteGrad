...
from . import util, dataset, models
amp = None
...
def val_epoch(model, loader, mel_idx, is_ext=None, n_test=1, get_output=False):
    ...
    if get_output:
        ...
        auc = roc_auc_score(
            (TARGETS == mel_idx).astype(float), PROBS[:, mel_idx])
        auc_20 = roc_auc_score((TARGETS[is_ext == 0] == mel_idx).astype(
            float), PROBS[is_ext == 0, mel_idx])
...
def run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, mel_idx):
    ...
    dataset_train = dataset.MelanomaDataset(
        df_train, 'train', meta_features, transform=transforms_train)
    dataset_valid = dataset.MelanomaDataset(
        df_valid, 'valid', meta_features, transform=transforms_val)
    train_loader = torch.utils.data.DataLoader(
        dataset_train, batch_size=args.batch_size, sampler=RandomSampler(dataset_train), num_workers=args.num_workers)
...
#     scheduler_cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.n_epochs - 1)
    ...
    scheduler_warmup = util.GradualWarmupSchedulerV2(
...
#         scheduler_warmup.step(epoch - 1)
        ...
        if auc > auc_max:
            print(
                'auc_max ({:.6f} --> {:.6f}). Saving model ...'.format(auc_max, auc))
            torch.save(model.state_dict(), model_file)
            auc_max = auc
        if auc_20 > auc_20_max:
            print(
                'auc_20_max ({:.6f} --> {:.6f}). Saving model ...'.format(auc_20_max, auc_20))
...
def main():
    ...
    df, df_test, meta_features, n_meta_features, mel_idx = dataset.get_df(
        args.kernel_type,
        args.out_dim,
        args.data_dir,
        args.data_folder,
    ...
    transforms_train, transforms_val = dataset.get_transforms(args.image_size)

    folds = [int(i) for i in args.fold.split(',')]
    for fold in folds:
        run(fold, df, meta_features, n_meta_features,
            transforms_train, transforms_val, mel_idx)
...
if __name__ == '__main__':
    ...
    if args.enet_type == 'resnest101':
        ModelClass = models.Resnest_Melanoma
    elif args.enet_type == 'seresnext101':
        ModelClass = models.Seresnext_Melanoma
