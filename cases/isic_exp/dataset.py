...
class MelanomaDataset(Dataset):
    def __init__(self, mode='valid', fold=0,  args=default_args.IsicAgrs()):
    ...
    def __getitem__(self, index):
        ...
        if self.use_meta:
            data = (torch.tensor(image).float(), torch.tensor(
                self.csv.iloc[index][self.meta_features]).float())
        else:
...
class MelanomaDatasetFx:

    def __init__(self, fold=0, size=3000, args=default_args.IsicAgrs()) -> None:
        val_dataset = MelanomaDataset(fold=fold, args=args)
        np.random.seed(0)
        if size is not None:
            selected = np.random.choice(len(val_dataset), size=size)
        else:
            selected = [i for i in range(len(val_dataset))]
        self.args = val_dataset.args
        self.class_label = val_dataset.csv['target'].to_numpy()[selected]
        self.val_dataset = torch.utils.data.Subset(val_dataset, selected)
        # {'AK': 0, 'BCC': 1, 'BKL': 2, 'DF': 3, 'SCC': 4, 'VASC': 5,
        #  'melanoma': 6, 'nevus': 7, 'unknown': 8}

        # https://challenge.isic-archive.com/landing/2019/
        self.classes_map = {
            0: {'encoded_label': '0', 'label': 'Actinic Keratosis (AK)'},
            1: {'encoded_label': '1', 'label': 'Basal Cell Carcinoma (BCC)'},
            # Benign keratosis (solar lentigo / seborrheic keratosis / lichen planus-like keratosis)
            2: {'encoded_label': '2', 'label': 'Benign Keratosis (BKL)'},
            3: {'encoded_label': '3', 'label': 'Dermatofibroma (DF)'},
            4: {'encoded_label': '4', 'label': 'Squamous Cell Carcinoma (SCC)'},
            5: {'encoded_label': '5', 'label': 'Vascular Lesion (VASC)'},
            6: {'encoded_label': '6', 'label': 'Melanoma'},
            7: {'encoded_label': '7', 'label': 'Melanocytic Nevus'},
            8: {'encoded_label': '8', 'label': 'Unknown'},
        }


class Isic2018Task1:

    def __init__(self, data_path='./data/isic-2018-task12',
                 image_size=512, args=default_args.IsicAgrs()) -> None:
        ...
        self.val_dataset = datasets.ImageFolder(
            os.path.join(data_path, 'ISIC2018_Task1-2_Test_Input'), transform=self.t)

        def seg_transform(ts: torch.Tensor):
            m = ts.mean(0)
            return torch.where(m > 0.5, 1, 0)
        ...
        self.val_seg_dataset = datasets.ImageFolder(
            os.path.join(data_path, 'ISIC2018_Task1_Test_GroundTruth'), transform=self.st)

        self.class_label = np.zeros(shape=len(self.val_dataset))
        self.classes_map = {
            0: {'encoded_label': '0', 'label': '0'},
        }
...
def get_transforms(image_size):
    ...
    transforms_train = albumentations.Compose([
        ...
        albumentations.Cutout(max_h_size=int(
            image_size * 0.375), max_w_size=int(image_size * 0.375), num_holes=1, p=0.7),
